...
- [Setup for AskUI Python SDK](#setup-for-askui-python-sdk)
...
To enable tracing in the AskUI Python SDK, you can configure the `OtelSettings` and pass it to the `act()` method via the `tracing_settings` parameter. See the [Setup for AskUI Python SDK](#setup-for-askui-python-sdk) section below for configuration details.

## Setup for AskUI Python SDK
   ...
   def get_tracing_settings() -> OtelSettings:
       return OtelSettings(
           ...
           endpoint=os.environ.get("OTEL_ENDPOINT", ""),
                      service_name="vision-agent-sdk",  # Optional: defaults to "askui-sdk"
...
- **`service_name`** (str): Name of your service in traces. Default: `"askui-sdk"`
