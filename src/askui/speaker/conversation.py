...
class Conversation:
    ...
    The Conversation holds the message history, truncation strategy, token usage,
    and current speaker. It orchestrates the conversation by delegating each step
    to the appropriate speaker.

    Args:
        speakers: Dictionary mapping speaker names to Speaker instances
        default_speaker: Name of the default speaker (e.g., "AskUIAgent")
    ...
    def __init__(
        ...
        """Initialize conversation with speakers and configuration."""
        if not speakers:
    ...
    def _handle_tool_results(
        ...
        if isinstance(tool_result_message.content, str):
            # tool_results = []
            return
        tool_results: list[ToolResultBlockParam] = tool_result_message.content  # type: ignore
        ...
        for tool_use_block, tool_result in zip(
            ...
            if (
                ...
                trajectory_file: str = tool_use_block.input["trajectory_file"]  # type: ignore
                start_from_step_index: int = tool_use_block.input.get(  # type: ignore
                    "start_from_step_index", 0
                )
                parameter_values: dict[str, str] = tool_use_block.input.get(  # type: ignore
                    "parameter_values", {}
                )
