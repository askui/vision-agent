CACHE_USE_PROMPT = (
    ...
    "- In some cases, the trajectories do not lead to the expected outcome. Hence, it"
    " is crucial that you check the message history and critically evaluate if it led"
    " to the goal specified by the user. Be careful, the steps executed by the"
    " CacheExecutor will appear as if you did them by yourself in the message history\n"
    "\n"
    "DYNAMIC PARAMETERS:\n"
    ...
    "- In these cases, the trajectory pauses at the non-cacheable steps, returning"
    " NEEDS_AGENT status with a current step index\n"
    "- If that happens, you need to first, critically evaluate, if the actions"
    " performed by the CacheExector so far did make sense or if it seems the"
    " CacheExecutor got stuck, e.g. only clicked in the middle of nowhere all the time."
    "- If everything looks good, execute the non-cacheable step manually and hand the"
    " execution back to the CacheExectur using the execute_cached_executions_tool with "
    " start_from_step_index parameter\n"
    "- If you observe that the cached execution did not work properly, execute all"
    " necessary steps to achieve the user goal manually, and do not resume the"
    " cached execution\n"
    "\n"
