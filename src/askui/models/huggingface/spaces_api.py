...
class HFSpacesHandler:
    def __init__(self) -> None:
        ...
        self.spaces: dict[str, Callable[[Image.Image, str, str | None], tuple[int, int]]] = {
    ...
    @staticmethod
    def _rescale_bounding_boxes(bounding_boxes, original_width, original_height, scaled_width = 1000, scaled_height = 1000):  # type: ignore
    ...
    def predict(self, screenshot: Image.Image, locator: str, model_name: str = "AskUI/PTA-1") -> tuple[int, int]:
        try:
            return self.spaces[model_name](screenshot, locator, model_name)
        except Exception as e:
            raise AutomationError(f"Hugging Face Spaces Exception: {e}")

    def predict_askui_pta1(self, screenshot: Image.Image, locator: str, model_name: str | None = None) -> tuple[int, int]:
    ...
    def predict_os_atlas(self, screenshot: Image.Image, locator: str, model_name: str | None = None) -> tuple[int, int]:
    ...
    def predict_qwen2_vl(self, screenshot: Image.Image, locator: str, model_name: str | None = None) -> tuple[int, int]:
        ...
        target_box = self._rescale_bounding_boxes(target_box, screenshot.width, screenshot.height)[0]
        assert len(target_box) == 4, f"Malformed box: {target_box}"
        x1, y1, x2, y2 = target_box
        x = int((x1 + x2) / 2)
        y = int((y1 + y2) / 2)
    ...
    def predict_showui(self, screenshot: Image.Image, locator: str, model_name: str | None = None) -> tuple[int, int]:
