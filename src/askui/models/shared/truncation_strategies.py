...
class TruncationStrategy(ABC):
    ...
    Args:
        max_messages: Maximum number of messages before forcing truncation.
        max_input_tokens: Maximum input tokens for the endpoint.
        truncation_threshold: Fraction of `max_input_tokens` at which to truncate.
    """
    ...
    def reset(self, messages: list[MessageParam] | None = None) -> None:
        ...
        Creates independent copies so modifications to truncated history
        do not affect full history.
...
class AskUITruncationStrategy(TruncationStrategy):
    """Truncation strategy that strips old images, manages cache breakpoints,
    and summarizes.

    On each appended message:

    1. Strips base64 images beyond `n_images_to_keep` (oldest first)
    2. Places dual cache breakpoints (at image-removal boundary + last user message)
    3. If token count exceeds threshold, summarizes the history via the VLM

    Args:
        vlm_provider: VLM provider used for summarization calls.
        n_images_to_keep: Number of most-recent base64 images to retain.
        n_messages_to_keep: Number of most-recent messages to preserve
    ...
    @override
    def append_message(self, message: MessageParam) -> None:
        """Append a message and apply image stripping, cache breakpoints,
        and truncation.
    ...
    @override
    def truncate(self) -> None:
        """Summarize old messages and keep only the most recent ones."""
        if len(self._truncated_message_history) <= self._n_messages_to_keep:
            msg = "Cannot truncate as there are too few messages in the history"
            logger.warning(msg)
            return

        summary = self._summarize_message_history()

        # Keep most recent messages
        recent = self._truncated_message_history[-self._n_messages_to_keep :]

        # Build new history starting with the summary as a user message
        new_messages: list[MessageParam] = [
            MessageParam(role="user", content=summary),
        ]

        # Ensure valid role alternation: if first recent message is also
        # "user", insert a synthetic assistant acknowledgement.
    ...
    def _remove_images(self) -> None:
        """Strip old base64 images from truncated history, keeping the most recent ones.

        Walks from the beginning of the message list and replaces excess
        base64 `ImageBlockParam` blocks with text placeholders.  Also
        recurses into `ToolResultBlockParam.content` lists.  URL-based
    ...
    def _count_base64_images(messages: list[MessageParam]) -> int:
        """Count total base64 image blocks across all messages."""
        count = 0
        for msg in messages:
            if isinstance(msg.content, str):
                continue
                ...
                if isinstance(block, ImageBlockParam) and isinstance(
                    ...
                    for nested in block.content:
                        if isinstance(nested, ImageBlockParam) and isinstance(
                            nested.source, Base64ImageSourceParam
                        ):
                            count += 1
        return count
    ...
    def _strip_base64_images(
        ...
        """Strip up to `max_to_strip` base64 images from a content block list.
        ...
        Returns:
            Tuple of (new content list, number of images stripped).
        """
        stripped = 0
    ...
    def _move_cache_breakpoints(self) -> None:
        ...
        - **Breakpoint 1** – at the image-removal boundary (stable prefix).
        - **Breakpoint 2** – on the last user message (recent context).
        """
    ...
    def _clear_cache_control(msg: MessageParam) -> None:
        """Clear ``cache_control`` on all blocks in a message."""
        if isinstance(msg.content, str):
            return
        cacheable = (
            ImageBlockParam,
        ...
        for block in msg.content:
            ...
            if isinstance(block, ToolResultBlockParam) and isinstance(
                block.content, list
            ):
                for nested in block.content:
    ...
    def _set_cache_breakpoint(msg: MessageParam) -> None:
        """Set a cache breakpoint on the last block of a message."""
    ...
    def _summarize_message_history(self) -> str:
        """Ask the VLM to summarize the conversation history.

        Returns:
            A summary string of the conversation so far.
        """
        messages_to_summarize = list(self._truncated_message_history)

        # Ensure valid role alternation before the summarization prompt
        if messages_to_summarize and messages_to_summarize[-1].role == "user":
            messages_to_summarize.append(
                MessageParam(role="assistant", content="I understand. Please go ahead.")
            )
        ...
        messages_to_summarize.append(
            MessageParam(
                ...
                content=(
                    ...
                    "history above. Focus on: key actions taken, results "
                    "observed, current state, and any pending tasks or "
                    "errors. This summary will replace the earlier "
                    "conversation history to save context space."
                ),
            )
        )

        response = self._vlm_provider.create_message(
