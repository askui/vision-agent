...
from askui.tools.android_agent_os import AndroidAgentOs
...
def _initialize_default_android_model_registry(  # noqa: C901
    os: AndroidAgentOs,
    reporter: Reporter,
) -> ModelRegistry:
    @functools.cache
...
class ModelRouter:
    ...
    @staticmethod
    def build_default_computer_router(
        tools: AgentToolbox,
        reporter: Reporter | None = None,
        models: ModelRegistry | None = None,
    ) -> "ModelRouter":
        """Build a default model router for computer agents."""
        if reporter is None:
            reporter = CompositeReporter()
        models_registry = _initialize_default_model_registry(tools, reporter)
        models_registry.update(models or {})
        return ModelRouter(models=models_registry)

    @staticmethod
    def build_default_android_router(
        os: AndroidAgentOs,
        reporter: Reporter | None = None,
        models: ModelRegistry | None = None,
    ) -> "ModelRouter":
        """Build a default model router for Android agents."""
        if reporter is None:
            reporter = CompositeReporter()
        models_registry = _initialize_default_android_model_registry(os, reporter)
        models_registry.update(models or {})
        return ModelRouter(models=models_registry)
