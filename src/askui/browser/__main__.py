...
def main():
    ...
    # Run command
    run_parser = subparsers.add_parser("run", help="Run a single task or start interactive mode")
    run_parser.add_argument("task", nargs="?", help="The task to execute. If omitted, starts interactive mode.")
    run_parser.add_argument("--url", help="Initial URL to navigate to")
    run_parser.add_argument("--headed", action="store_true", default=True, help="Run in headed mode (default)")
    run_parser.add_argument("--headless", action="store_false", dest="headed", help="Run in headless mode")
    run_parser.add_argument("--verbose", action="store_true", help="Show detailed agent thinking and heuristics")
    ...
    workflow_subparsers = workflow_parser.add_subparsers(dest="subcommand", help="Workflow subcommands")

    record_parser = workflow_subparsers.add_parser("record", help="Record a new workflow")
    ...
    replay_parser = workflow_subparsers.add_parser("replay", help="Replay a saved workflow")
    replay_parser.add_argument("input", help="Input JSON file of the workflow")
    replay_parser.add_argument("--headed", action="store_true", default=True, help="Run in headed mode (default)")
    replay_parser.add_argument("--headless", action="store_false", dest="headed", help="Run in headless mode")
    replay_parser.add_argument("--verbose", action="store_true", help="Show detailed agent thinking")
