...
class AndroidScreenshotTool(Tool):
    ...
    def __init__(self, os_agent_handler: AndroidAgentOSHandler) -> None:
        super().__init__(
        ...
        self._os_agent_handler = os_agent_handler

    @override
    def __call__(self) -> tuple[str, Image.Image]:
        screenshot = self._os_agent_handler.screenshot()
...
class AndroidTapTool(Tool):
    ...
    def __call__(self, x: int, y: int) -> str:
        self._os_agent_handler.tap(x, y)
...
class AndroidTypeTool(Tool):
    ...
    def __call__(self, text: str) -> str:
        self._os_agent_handler.type(text)
        return f"Typed: {text}"
...
class AndroidDragAndDropTool(Tool):
    ...
    def __call__(self, x1: int, y1: int, x2: int, y2: int, duration: int = 1000) -> str:
        self._os_agent_handler.drag_and_drop(x1, y1, x2, y2, duration)
...
class AndroidKeyTapEventTool(Tool):
    ...
    def __call__(self, key_name: ANDROID_KEY) -> str:
        self._os_agent_handler.key_tap(key_name)
...
class AndroidSwipeTool(Tool):
    ...
    @override
    def __call__(self, x1: int, y1: int, x2: int, y2: int, duration: int = 1000) -> str:
        self._os_agent_handler.swipe(x1, y1, x2, y2, duration)
...
class AndroidKeyCombinationTool(Tool):
    ...
    def __call__(self, keys: list[ANDROID_KEY], duration: int = 100) -> str:
        self._os_agent_handler.key_combination(keys, duration)
...
class AndroidShellTool(Tool):
    ...
    def __call__(self, command: str) -> str:
        output = self._os_agent_handler.shell(command)
