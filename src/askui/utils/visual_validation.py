...
def compute_phash(image: Image.Image, hash_size: int = 8) -> str:
    ...
    Example:
        ...
        >>> hash1 = compute_phash(img, hash_size=8)
        >>> hash2 = compute_phash(img, hash_size=8)
        >>> assert hash1 == hash2  # Same image produces same hash
    """
...
def compute_ahash(image: Image.Image, hash_size: int = 8) -> str:
    ...
    Example:
        >>> from PIL import Image
        >>> img = Image.open("screenshot.png")
        >>> hash1 = compute_ahash(img, hash_size=8)
        >>> hash2 = compute_ahash(img, hash_size=8)
        >>> assert hash1 == hash2  # Same image produces same hash
    """
...
def extract_region(
    ...
    Example:
        ...
        >>> action_input = {"action": "click", "coordinate": [50, 50]}
        >>> region = extract_region(img, action_input, region_size=20)
        >>> assert region.size == (20, 20)
    """
    coordinate = action_input.get("coordinate")
    if not coordinate:
        logger.warning("No coordinate found in action_input")
        return image
...
def find_recent_screenshot(
    ...
    Example - During recording:
        >>> # Find screenshot before tool use at index i
